...
- ``write_file``: always sandboxed to the workspace — absolute paths are rejected.
"""
...
def write_file(filepath: str, content: str, workspace_dir: str = "") -> str:
    """Write content to a file within the workspace (sandboxed).

    Only relative paths are accepted — absolute paths are rejected for safety.
    ...
    try:
        if _is_absolute_path(filepath):
            return "Error: write_file only accepts relative paths within the workspace"
        target = _resolve_safe(ws, filepath)
